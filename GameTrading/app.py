...
class UserCollection(Resource):
    ...
    def post(self):
        ...
        try:
            validate(
                request.json, self.json_schema(), format_checker=draft7_format_checker
            )
        ...
        db_key = ApiKey(key=ApiKey.key_hash(token), user=user)
        print(token)  # For dev purposes
        db.session.add(user)
    ...
    def get(self):
        ...
        collection = []
        for u in users:
            collection.append({"id": u.id, "username": u.username, "email": u.email})
    ...
    def json_schema():
        ...
        properties["username"] = {"type": "string", "maxLength": 40}
        properties["email"] = {"type": "string", "format": "email", "maxLength": 40}
        properties["password"] = {"type": "string", "maxLength": 40}
        return schema
...
class UserGameCollection(Resource):
    """
    ...
    def json_schema():
        ...
        properties["description"] = {"type": "string", "maxLength": 700}
        properties["is_digital"] = {"type": "boolean"}
            ...
            collection.append(
                {
                    "id": t.id,
                    "status": t.status,
                    "sender_game_id": t.sender_game_id,
                    "receiver_game_id": t.receiver_game_id,
                }
            )
        return collection, 200
...
class UserTradeCollection(Resource):
    ...
    def post(self, user):
        ...
        trade_exist = Trade.query.filter_by(
            sender_game_id=sender_game.id, receiver_game_id=receiver_game.id
        ).first()
    ...
    def json_schema():
        ...
        schema = {"type": "object", "required": ["sender_game_id", "receiver_game_id"]}
        properties = schema["properties"] = {}
        properties["sender_game_id"] = {"type": "integer"}
        properties["receiver_game_id"] = {"type": "integer"}
        return schema
...
class UserTradeItem(Resource):
    """
    Resouce for trding item
    """

    @require_user_key
    def put(self, user, trade):
        """
...
api.add_resource(UserGameItem, "/api/users/<user:user>/games/<game:game>/")

api.add_resource(UserTradeCollection, "/api/users/<user:user>/trades/")
api.add_resource(UserTradeItem, "/api/users/<user:user>/trades/<trade:trade>/")
